...
import { Wallet, CircleDollarSign, AlertOctagon, Upload, ChevronLeft, ChevronRight, ArrowUpDown } from "lucide-react";
...
export default function StudentPayment() {
  ...
  const handleUpload = async (studentFeeId: number, amount: number) => {
    setUploading(studentFeeId);
    await fetch("/api/student/payments", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ studentFeeId, amountPaid: amount, bankSlipUrl: null }),
    });
    setUploading(null);
    load(); // refresh
  };
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/40 text-left text-xs uppercase tracking-wider text-muted-foreground">
                {["Fee Type", "Due Date", "Penalty", "Amount", "Status", "Slip Upload", "Approval"].map((h) => (
              ...
              {loading ? (
                ...
                <tr key={f.studentFeeId} className="border-b last:border-0 transition hover:bg-muted/30">
                  ...
                  <td className="px-6 py-4">
                    <button
                      disabled={uploading === f.studentFeeId || f.status === "Paid"}
                      onClick={() => handleUpload(f.studentFeeId, f.amount)}
                    ...
                    >
                      ...
                      {uploading === f.studentFeeId ? "Submitting..." : "Upload"}
                    </button>
                  </td>
                  <td className="px-6 py-4">
